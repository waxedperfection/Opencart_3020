...
class ControllerExtensionModuleAmazonLogin extends Controller {
	...
	public function index() {
		...
		if (isset($this->request->post['module_amazon_login_button_type'])) {
			$data['amazon_login_button_type'] = $this->request->post['module_amazon_login_button_type'];
		} elseif ($this->config->get('module_amazon_login_button_type')) {
			$data['amazon_login_button_type'] = $this->config->get('module_amazon_login_button_type');
		} else {
			$data['amazon_login_button_type'] = 'LwA';
		}

		if (isset($this->request->post['module_amazon_login_button_colour'])) {
			$data['amazon_login_button_colour'] = $this->request->post['module_amazon_login_button_colour'];
		} elseif ($this->config->get('module_amazon_login_button_colour')) {
			$data['amazon_login_button_colour'] = $this->config->get('module_amazon_login_button_colour');
		} else {
			$data['amazon_login_button_colour'] = 'gold';
		}

		if (isset($this->request->post['module_amazon_login_button_size'])) {
			$data['amazon_login_button_size'] = $this->request->post['module_amazon_login_button_size'];
		} elseif ($this->config->get('module_amazon_login_button_size')) {
			$data['amazon_login_button_size'] = $this->config->get('module_amazon_login_button_size');
		} else {
			$data['amazon_login_button_size'] = 'medium';
		}

		if (isset($this->request->post['module_amazon_login_status'])) {
			$data['amazon_login_status'] = $this->request->post['module_amazon_login_status'];
		} else {
			$data['amazon_login_status'] = $this->config->get('module_amazon_login_status');
		}

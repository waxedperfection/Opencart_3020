...
{country}';
	...
	private function getCategoryStructure($id) {
		$res = $this->db->query("SELECT * FROM `" . DB_PREFIX . "ebay_category` WHERE `CategoryID` = '" .(int)$id . "' LIMIT 1");
		return $res->row;
	}
	...
	public function getProductStock($id) {
		$res = $this->db->query("SELECT * FROM `" . DB_PREFIX . "product` WHERE `product_id` = '" .(int)$id . "' LIMIT 1");
